...
// // Function to determine if a status item is failing
// function isFailingStatus(statusItem) {
//   const icon = statusItem.querySelector(".merge-status-icon svg");
//   return icon && icon.classList.contains("color-fg-danger");
// }

// // Function to sort status items
// function sortStatusItems(container) {
//   const statusItems = Array.from(container.children);
//   const failingItems = statusItems.filter(isFailingStatus);
//   const passingItems = statusItems.filter((item) => !isFailingStatus(item));

//   // Remove all items from the container
//   statusItems.forEach((item) => item.remove());

//   // Add failing items first, then passing items
//   failingItems.forEach((item) => container.appendChild(item));
//   passingItems.forEach((item) => container.appendChild(item));
// }
